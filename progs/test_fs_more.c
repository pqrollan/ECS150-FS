...
{
	struct thread_arg *t_arg = arg;
	char *diskname, *tofile, *fromfile, *offsetstr, *buf;
	int fd, fs_fd;
        size_t offset;
	struct stat st;
	int written;
	...
	if (t_arg->argc < 4)
		die("Usage: <diskname> <host filename> <host filename> <offset>");

	diskname = t_arg->argv[0];
	tofile = t_arg->argv[1];
        fromfile = t_arg->argv[2];
        offsetstr = t_arg->argv[3];

        offset = (size_t)strtol(offsetstr, &wasteptr, 10);
	...
	printf("Wrote to file '%s' from '%s' (%d/%zu bytes)\n", tofile,
                fromfile, written, st.st_size);
